...
class RequestRemoveViewModel extends GetxController {
  final _api = RequestRemoveRepository();

  UserPreference userPreference = UserPreference();

  RxString? request_from_id = ''.obs;
  RxBool loading = false.obs;
  void requestRemoveApi() async {
    ...
    _api.requestRemoveApi(head, body).then((value) async {
      loading.value = false;

      if (value['status'] == 'failed') {
        Utils.toastMessageCenter(value['message'], true);
      } else {
        Get.delete<RequestRemoveViewModel>();
        // var requestList = Get.put(RequestListViewModel());
        // requestList.refreshApi();
